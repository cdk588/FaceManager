package com.cdk.facemanager.faceserver;
...
import com.cdk.facemanager.common.Constants;
import com.cdk.facemanager.model.FaceRegisterInfo;
...
//                int engineCode = faceEngine.init(context, FaceEngine.ASF_DETECT_MODE_IMAGE, FaceEngine.ASF_OP_0_ONLY, 16, 1, FaceEngine.ASF_FACE_RECOGNITION | FaceEngine.ASF_FACE_DETECT);
    ...
    public void activeEngine(final Context context) {
        ThreadUtils.runOnSubThread(new Runnable() {
            ...
            public void run() {
                ...
                final int activeCode = faceEngine.activeOnline(context, Constants.APP_ID, Constants.SDK_KEY);
...
//                            Toast.makeText(context, "引擎已激活，无需再次激活", Toast.LENGTH_SHORT).show();
                            LogUtils.e("引擎已激活，无需再次激活");
                        } else {
                            Toast.makeText(context, "引擎激活失败，错误码为 " + activeCode, Toast.LENGTH_SHORT).show();
                            LogUtils.e("引擎激活失败，错误码为 " + activeCode);
                        }
